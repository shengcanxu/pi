...
type RtkUnavailableReason = "missing" | "unexecutable" | "timeout";

type SpawnErrorClassification = RtkUnavailableReason | "other";

/**
 * Pure helpers that do not depend on session state.  They stay at module
 * scope so they are shared across any re-load of the extension.
 */
...
export default function (pi: ExtensionAPI) {
  // Session state is intentionally in-memory only: it resets to enabled on
  // every Pi process start and is never persisted to disk.  Keeping state
  // inside the extension closure also isolates it across extension re-loads.
  let sessionEnabled = true;

  // Availability notifications are warn-once per outage: a successful rewrite
  // spawn resets the gate, and the next ENOENT/EACCES/ETIMEDOUT may warn
  // again.  Pi only exposes the TUI notify surface through lifecycle context,
  // so the callable is captured from the first relevant event rather than at
  // module load.
  ...
  function alertRtkUnavailable(reason: RtkUnavailableReason): void {
    ...
    const messages: Record<RtkUnavailableReason, string> = {
      ...
      timeout:
        "[pi-rtk] rtk process timed out. Shell command rewrites are disabled. Check whether rtk is hung or unresponsive.",
    };
  ...
  function rtkRewriteCommand(command: string): string | undefined {
    ...
    // Spawn availability errors are handled above by the transition-based
    // notify gate.
    try {
  ...
  function rtkStatusReport(ctx: ExtensionContext): StatusReport {
    ...
    // Single spawn to avoid serial 2 × timeout blocking (STATUS_TIMEOUT_MS
    // is shorter than REWRITE_TIMEOUT_MS because status is user-facing and
    // should never feel sluggish).
    const result = spawnSync(
      "sh",
      [
        "-c",
        // Use a null-byte separator so the two values can never collide with
        // rtk --version output.
        'v=$(rtk --version 2>/dev/null); p=$(command -v rtk 2>/dev/null); printf "%s\\0%s" "$v" "$p"',
      ],
    ...
    if (result.error) {
      ...
      const versionText = (parts[0] ?? "").trim() || "version unknown";
      const pathText = (parts[1] ?? "").trim();

      if (pathText.length > 0) {
        binary = `${versionText} at ${pathText}`;
